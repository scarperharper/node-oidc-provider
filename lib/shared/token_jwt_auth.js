...
module.exports = function getTokenJwtAuth(provider) {
  ...
  return async function tokenJwtAuth(ctx, keystore, algorithms) {
    ...
    if (Array.isArray(payload.aud)) {
      ...
      throw new InvalidClientAuth(`audience (aud) must equal the endpoint url, issuer identifier or token endpoint url. aud is '${payload.aud}', acceptedAud is '${Array.from(acceptedAud).join()}'`);
    }
